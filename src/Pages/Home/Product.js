  ...
  const { data: user, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://trusted-furniture-server.vercel.app/user?email=${email}`
      );
      ...
      <div className="card-body lg:w-2/3">
        <div>
          ...
          <div className="flex">
            ...
            {user?.verified === true && (
              <FaCheck className="bg-blue-600 rounded-full" />
            )}
          </div>
        </div>
        ...
        <div className="card-actions justify-end">
          ...
          <button onClick={() => handleWishlist(_id)}>
            <span className="text-5xl ">
              <FaHeart className="text-orange-300 hover:text-amber-600"></FaHeart>
            </span>
          </button>
          <label
            onClick={() => setProductItem(product)}
            htmlFor="booking-modal"
            className="bg-orange-300 p-2 rounded-md cursor-pointer hover:bg-amber-400"
          >
            Book Now
          </label>
        </div>
